...
//dbg information
	...
	if ((!(BB == &F->front())) && (!(BB->hasNPredecessorsOrMore(1)))){
	  continue;
	}
    ...
    while (!worklist.empty())
    {
        ...
        worklist.pop_front();
	//errs()<<"== "<<visitOrder++<<" : Work on Basic Block : "<<BB->getName().str()<<"\n";
		    ...
		    if (isTerminationBB(pred)) {
		   	    OP<<"pred is termination.";
                    ...
                    if (!init)
                    {
                        ...
                        in.assign(outQualiArray[pred].begin(), outQualiArray[pred].end());
                        init = true;
                    }
                    else
                    {
                        qualiJoin(in, outQualiArray[pred], numNodes);
                    }
                }
            }
	...
	//fSummary.summary();
		...
		//OP<<"obj = "<<obj<<"\n";
				...
				if (idx != AndersNodeFactory::InvalidIndex) {
				    for (auto relatedObj : fSummary.sumPtsGraph[idx]) {
                                        fSummary.args[sumObj - sumObjOffset + i].addToRelatedArg(relatedObj-sumObjOffset + i);
                                        //OP<<"3fSummary.args["<<sumObj - sumObjOffset + i<<"].addToRelatedArg("<<relatedObj-sumObjOffset + i<<")\n";
                                    }
				}
                            }
                        }
    ...
    eTime = clock();
    OP<<"Time for summarizeFuncs "<<(double)(eTime - sTime) / CLOCKS_PER_SEC<<"\n";	
...
// %src-para && %dst-para
	...
	//old impl to check if the obj is union
	...
	int qualiA = in.at(op0Index), qualiB = in.at(op1Index);
	...
	if (qualiA == _UD || qualiB == _UD) {
	    out.at(dstIndex) = _UD;
	}
	else {
	    out.at(dstIndex) = std::min(qualiA, qualiB);
	}
