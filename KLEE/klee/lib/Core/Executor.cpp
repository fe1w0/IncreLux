...
void Executor::initializeGlobals(ExecutionState &state) {
  ...
  for (Module::const_global_iterator i = m->global_begin(),
    ...
    if (i->hasInitializer()) {
     ...
     // if (i->isConstant())  constantObjects.emplace_back(wos);
    }
  }
...
//                           CallSite(cast<CallInst>(caller)));
  ...
  case Instruction::Call: {
    ...
    llvm::raw_string_ostream rso(ld);
    i->print(rso);
    if (ld.find("llvm.read_register") != std::string::npos || ld.find("llvm.write_register")!= std::string::npos)
	break;

    if (ld.find("frameaddress") != std::string::npos)
	break; 
//    CallSite cs(i);
    ...
    for (unsigned j=0; j<numArgs; ++j)
      arguments.push_back(eval(ki, j+1, state).value);
                ...
                auto ff = b->getParent();
                std::string fname = ff->getName().str();
